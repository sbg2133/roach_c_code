...
static double accum_len = ( 1 << 20 ) - 1;
double freqs[2] = {10.3285242e6,-70.0832511e6};
size_t freqlen = 2;
const char *roach1_fpg = "roach1.fpg";
const char *roach2_fpg = "roach2.fpg";
const char *roach3_fpg = "roach3.fpg";
const char *roach4_fpg = "roach4.fpg";
...
static void roach_select_bins(roach_state_t *m_roach, double *m_freqs, size_t m_freqlen)
{
	...
	printf("\nbin, fbin, freq, freq_res: %d, %g, %0.9g, %0.9g\n", bins[i], bin_freqs[i], m_freqs[i]/1.0e6, (m_freqs[i] - bin_freqs[i])/1.0e6);
...
void save_luts(roach_state_t *m_roach)
{
	...
	for (size_t i = 0; i < m_roach->lut_buffer_len; i++){
    		fprintf(f1,"%g\n", m_roach->DAC.I[i]); 
    		fprintf(f2,"%g\n", m_roach->DAC.Q[i]); 
    		fprintf(f3,"%g\n", m_roach->DDS.I[i]); 
    		fprintf(f4,"%g\n", m_roach->DDS.Q[i]); 
...
void save_packed_luts(roach_state_t *m_roach)
{
	...
	for (size_t i = 0; i < 2*m_roach->lut_buffer_len; i++){
...
void write_tones(roach_state_t *m_roach, double *m_freqs, size_t m_freqlen)
{	
	printf("Allocating memory for LUTs...\n");
	roach_init_DACDDS_LUTs(m_roach, m_roach->lut_buffer_len);
...
void save_packets(size_t m_num_packets, int m_sock_desc, double m_filetag, const char *m_savepath, const char *m_pathtag)
{	
...
void sweep_lo(double m_centerfreq, double m_span, double delta_f, int m_sock_desc, bool save, const char *m_savepath, const char *m_filetag)
{
...
void roach_sweep(roach_state_t *m_roach, double m_min_freq, double m_max_freq, size_t m_freqlen, double m_centerfreq, const char *m_savepath, const char *m_packetdir, int m_sock_desc, 
									bool vna, bool write, bool plot)
{
	if (vna) {
		...
		if (write) {
			write_tones(m_roach, m_roach->freq_comb, m_roach->freqlen);
		}	
		...
		// reorder frequencies
		if (write) {
			write_tones(m_roach, m_kid_freqs, m_roach->num_kids);
		}
...
{	
	...
	roach2.dac_freq_res = fpga_samp_freq / roach2.lut_buffer_len ;
	...
	char option;
		...
		case '2':
			...
			write_tones(&roach2, roach2.freq_comb, roach2.freqlen);	

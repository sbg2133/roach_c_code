...
class FirmwareSnaps(object):
	...
	def plotLPF(self):
		...
		ax.spines['right'].set_color('none')
		#ax.suptitle('ADC timestream', size = 20)
		ax.set_xlabel('Clock cycles (1/512 MHz =  2 ns)', size = 20)
		ax.set_ylabel('Amplitude', size = 20)
		plot1 = fig.add_subplot(211)
		line1, = plot1.plot(np.arange(0,2048), np.zeros(2048), 'r-', linewidth = 2)
		plot1.set_title('I', size = 20)
		plt.xlim(0,1024)
		plt.ylim(-2**16 - 1, 2**16 - 1)
		#plt.ylim(-2**12 - 1,2**12 -1)
		plt.grid()
		plot2 = fig.add_subplot(212)
		line2, = plot2.plot(np.arange(0,2048), np.zeros(2048), 'b-', linewidth = 2)
		...
		while count < stop:    
			...
			self.fpga.write_int('adc_lpf_trig',0)    
			self.fpga.write_int('adc_lpf_trig',1)    
			self.fpga.write_int('adc_lpf_trig',0)
			#lpf = (np.fromstring(self.fpga.read('lpf_bram',(2**11)*4),dtype='>h')).astype('float')
			lpf = (np.fromstring(self.fpga.read('lpf_bram',(2**12)*16),dtype='>i')).astype('float')
			#lpf[ lpf > 2**15 - 1] -= (2**15 - 1)
			#lpf /= (2**15 -1)
			...
			#I = lpf[0::2]
			#Q = lpf[1::2]
			I = np.hstack(zip(lpf[0::4],lpf[1::4]))
			Q = np.hstack(zip(lpf[2::4],lpf[3::4]))
			line1.set_ydata(I)
			line2.set_ydata(Q)
			plt.draw()
			count += 1
	...
	def plotFFT(self):
		...
		while(count < stop):
			...
			fft_snap = (np.fromstring(self.fpga.read('fft_snap_bram',(2**9)*8),dtype='>h')).astype('float')
			I0 = fft_snap[0::4]
			Q0 = fft_snap[1::4]
			I1 = fft_snap[2::4]
			Q1 = fft_snap[3::4]
	...
	def plotMixer(self, chan):
		...
		while (count < stop):
			...
			line5.set_ydata(mag)
			plot3.set_ylim(( np.min(mag) - 20 ,np.max(mag) + 20))
	...
	def plotPhase(self, chan):
		...
		while 1: 
			for count in range(100):
				...
				plot1.set_ylim((np.min(chan_I),np.max(chan_I)))
				plot2.set_ylim((np.min(chan_Q),np.max(chan_Q)))
				plt.draw()	
